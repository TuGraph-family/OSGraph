...
class ProjectContributionServiceConfig(ServiceConfig):
    def __init__(self):
        super().__init__(
            ...
            filterKeys=[
                FilterKey(key="start-time", type="int", default=get_default_start_time(), required=False),
                FilterKey(key="end-time", type="int", default=get_default_end_time(), required=False),
                FilterKey(key="contribution-limit", type="int", default=50, required=False)
            ]
        )
