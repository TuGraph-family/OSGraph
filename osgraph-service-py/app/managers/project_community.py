...
from app.models.graph_view import Graph, Repo, Company, Country, User, PullRequestAction, Star, Belong
        ...
        if result:
            for data in result:
                ...
                if start_node["type"] == 'country':
                    country = Country(id=start_node["id"],name=start_node["properties"]["name"])
                    graph.insert_entity(country)
                if start_node["type"] == 'company':
                    company = Repo(id=start_node["id"],name=start_node["properties"]["name"])
                ...
                if end_node["type"] == 'github_repo':
                    ...
                    graph.insert_entity(repo)
                if start_node["type"] == 'country':
                    country = Country(id=end_node["id"],name=end_node["properties"]["name"])
                    graph.insert_entity(country)
                if start_node["type"] == 'company':
                    company = Repo(id=end_node["id"],name=end_node["properties"]["name"])
                    graph.insert_entity(company)

                if relationship["type"] == "PR":
                    pr = PullRequestAction(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                    graph.insert_relationship(pr)
                if relationship["type"] == "Star":
                    star = Star(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                    graph.insert_relationship(star)
                if relationship["type"] == "belong_to":
                    belong = Belong(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"])
