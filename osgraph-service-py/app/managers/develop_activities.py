...
from app.models.graph_view import Graph, User, Repo, Push, CreatePR, CodeReviewAction, CreateIssue, CommentIssue
        ...
        if result:
            for data in result:
                ...
                if start_node["type"] == 'github_repo':
                    repo = Repo(id=start_node["id"],name=start_node["properties"]["name"])
                ...
                if relationship["type"] == "push":
                    push = Push(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                    graph.insert_relationship(push)

                if relationship["type"] == "open_pr":
                    open_pr = Push(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                    graph.insert_relationship(open_pr)

                if relationship["type"] == "code_review":
                    code_review = Push(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                    graph.insert_relationship(code_review)

                if relationship["type"] == "open_issue":
                    open_issue = Push(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                    graph.insert_relationship(open_issue)

                if relationship["type"] == "comment_issue":
                    comment_issue = Push(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
