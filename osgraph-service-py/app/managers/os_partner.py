...
from app.models.graph_view import Graph, User, CommonIssue, CommonPR, CommonStar, CommonRepo
            ...
            if result:
                for data in result:
                    ...
                    if start_node["type"] == 'github_user':
                        src_user = User(id=start_node["id"],name=start_node["properties"]["name"])
                        graph.insert_entity(src_user)
                
                    if end_node["type"] == 'github_user':
                        tar_user = User(id=end_node["id"],name=end_node["properties"]["name"])
                        graph.insert_entity(tar_user)
                    
                    if end_node["type"] == 'common_issue':
                        common_issue = CommonIssue(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                        graph.insert_entity(common_issue)

                    if relationship["type"] == "common_pr":
                        common_pr = CommonPR(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"],count=relationship["properties"]["count"])
                        graph.insert_relationship(common_pr)

                    if relationship["type"] == "common_star":
                        common_star = CommonStar(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"], count=relationship["properties"]["count"])
                        graph.insert_relationship(common_star)

                    if relationship["type"] == "common_repo":
                        common_repo = CommonRepo(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"], count=relationship["properties"]["count"])
