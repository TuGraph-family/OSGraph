        ...
        if result:
            for data in result:
                ...
                if start_node["type"] == 'topic':
                    topic = Topic(id=start_node["id"],name=start_node["properties"]["name"])
                ...
                if end_node["type"] == 'topic':
                    topic = Topic(id=end_node["id"],name=end_node["properties"]["name"])
                    graph.insert_entity(topic)
                ...
                if relationship["type"] == "repo":
                    contribute_repo = ContributeRepo(sid=relationship["src"],tid=relationship["dst"],id=relationship["id"], count=relationship["properties"]["count"])
